...
public class MetalDetectorItem extends Item {
        ...
        if(!level.isClientSide()) {
            ...
            for(int i = 0; i <= positionClicked.getY() + 64; i++) {
                ...
                if(isValueableBlock(blockState)) {
                    outputValueableCoordinates(positionClicked.below(i), player, blockState.getBlock());
                    foundBlock = true;
            ...
            if(!foundBlock) {
                outputNoValueablesFound(player);
            }
        }
    ...
    private void outputNoValueablesFound(Player player) {
        player.sendSystemMessage(Component.translatable("item.alittlebitofeverything.metal_detector.no_valueables"));
    }

    private void outputValueableCoordinates(BlockPos position, Player player, Block block) {
        player.sendSystemMessage(Component.literal("Valueables found ")
